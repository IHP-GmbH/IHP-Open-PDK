...
WORK=${WORK:-$pdir/work}
mkdir -p "$WORK"
WORK=$(cd "$WORK" && pwd)  # stages cd around, keep it absolute
OUT=${OUT:-$pdir/$pname.$CORNER.rules}
EPS=${EPS:-4.1}
EXT=${EXT:-20}
...
solve_one() {
    ...
    python3 "$solve" "$1" --process "$PROCESS" --fastercap "$FASTERCAP" \
        ...
        > "$1/solve.log" 2>&1 || { rc=$?; echo "FAILED $1" >> "$1/solve.log"; }
...
fastercap() {
    ...
    # xargs exits 123 if any worker failed; stop before parse/model would
    # silently build an incomplete model
    xargs -r -P "$JOBS" -I{} bash -c 'solve_one "$1"' _ {} < "$todo" || {
        echo "$(grep -l '^FAILED' $(sed 's|$|/solve.log|' "$todo") | wc -l)" \
             "patterns failed, see <pattern>/solve.log; rerun to retry them" >&2
        exit 1
    }
}
