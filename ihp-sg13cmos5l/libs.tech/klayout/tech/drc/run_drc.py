...
# =========================================================================================
#
# Modified for IHP-SG13CMOS5L slim PDK (CMOS-only, M1-M5)
...
def generate_klayout_switches(arguments, layout_path: str) -> dict:
    ...
    # Locate JSON rule file paths (MODIFIED FOR SG13CMOS5L)
    script_dir = Path(__file__).resolve().parent
    tech_rule_path = (
        script_dir / "../../python/sg13cmos5l_pycell_lib/sg13cmos5l_tech_mod.json"
    ).resolve()
    default_rule_path = (script_dir / "rule_decks/sg13cmos5l_tech_default.json").resolve()
...
def run_check(
    ...
    logging.info(
        f"Running IHP-SG13CMOS5L {' '.join(drc_tables)} checks on design {layout_path}, topcell: {topcell}"
    )
...
def run_parallel_run(
    ...
    # Note: sg13cmos5l_maximal.drc not available in slim PDK - extra rules disabled
    ...
    for table in table_list:
        rule_deck_files[table] = rule_deck_full_path / "ihp-sg13cmos5l.drc"
...
def run_single_processor(
    ...
    # Run primary table check (MODIFIED FOR SG13CMOS5L)
    ...
    if "main" not in tables:
        ...
        switches["no_forbidden"] = "true"
        switches["no_pin"] = "true"
    result_dbs.append(run_check(rule_deck_full_path / "ihp-sg13cmos5l.drc", tables, layout_path, run_dir, switches))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run IHP SG13CMOS5L OpenSource DRC checks (Slim PDK - CMOS only, M1-M5)",
        usage=USAGE,
    )
    ...
    parser.add_argument(
        "--disable_extra_rules",
        action="store_true",
        help="Disable the remaining DRC rules from the full rule set (not applicable in slim PDK).",
    )
    parser.add_argument(
