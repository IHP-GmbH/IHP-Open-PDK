...
########################################################################
#
# Generator for every cap_cmomf LVS testcase layout in this tree, including the
...
#       cmomf_mirrored.gds     the same chain with the middle cap mirrored
...
# (Mn.a in the DRM), and every routed layout here is on Metal4.
WIRE = 0.20
...
# which of the deck's m1p..m4p port paths is exercised at all. mmax=1 also puts
# them on the 0.16 wide Metal1 bar, the one case where the PCell has to clip the
# pin box to the metal it labels.
CONFIG_MATRIX = [
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 4},   # ports on Metal4
    {"w": 5e-6, "l": 5e-6, "mmin": 2, "mmax": 4},   # ports on Metal4, band off Metal1
    ...
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 4, "subblock": 1},  # PWell.block variant
    {"w": 10e-6, "l": 20e-6, "mmin": 1, "mmax": 4},  # non-square, exercises l->X / w->Y
]


def build_original():
    """The original cap_cmomf testcase, rebuilt from the PCell.

    It used to be a frozen GDS with no recorded recipe, the same state issue #91
    described for cap_cmomi: the geometry really is the PCell's output, but
    nothing said at which parameters, so it could not be regenerated after a
    PCell change and a drift would have gone unnoticed.

    Same intent as before, so cap_cmomf.cdl is unchanged: three caps covering
    both orientations of the top metal plus a stack that stops below Metal4.
    The finger direction alternates per layer, so the metal that carries the pins
    and the shape of the bar they land on both depend on how many layers the
    stack has.

    Flattened, unlike the copy it replaces, which held its three devices in
    sub-cells. A cap_cmomf alone in a sub-cell extracts nothing in deep mode
    ...
    cells = [
        _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4),
        _pcell(layout, w=5e-6, l=5e-6, mmin=2, mmax=4),
...
def build_hier():
    ...
    device; see the cap_cmomi section of testing/README.md and issue #91. An
    empty netlist is reported as a mismatch now, so this cannot be a unit
    testcase in deep mode, and the checks suite runs it there instead.
    """
    layout = _new_layout()
    top = layout.create_cell("cap_cmomf_hier")

    pcell = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4)
...
def build_hier_wired():
    ...
    Two instances of a sub-cell holding one cap, joined by a Metal4 route drawn
    ...
    unit.insert(pya.DCellInstArray(
        _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4), pya.DTrans()))
    unit.flatten(-1, True)
...
def _chain(layout, top, mirrored=None):
    ...
    cells = [_pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4) for _ in range(3)]
...
def build_shorted():
    ...
    cells = [_pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4) for _ in range(2)]
...
def build_coexist():
    """One cap_cmomf beside one cap_cmomi, both on Metal1..Metal4.
    ...
    cells = [
        _pcell(layout, w=4e-6, l=12e-6, mmin=1, mmax=4),
        _pcell(layout, name="cap_cmomi", w=4e-6, l=12e-6, mmin=1, mmax=4, feed="double"),
    ]
    _row(layout, top, cells)
    top.flatten(-1, True)
