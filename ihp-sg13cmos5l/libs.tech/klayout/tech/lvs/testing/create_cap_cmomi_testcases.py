...
########################################################################
#
# Generator for every cap_cmomi LVS testcase layout in this tree, including the
# original one, which used to be a frozen copy and is now rebuilt from the PCell.
#
...
#       cap_cmomi.gds          the original testcase, rebuilt from the PCell
...
#
# KLAYOUT_HOME matters: a user KLayout home that has ihp-sg13cmos5l installed
# registers its own 'sg13cmos5l' technology with a base path pointing at that
...
METAL_LAYER = {1: 8, 2: 10, 3: 30, 4: 50}
VIA_LAYER = {1: 19, 2: 29, 3: 49}  # VIAn connects Metal n to Metal n+1
RECOG_MOM = (99, 39)

# Top metal of the devices the checks suite builds (mmax of their band). This
# PDK stops at Metal4: cmos5l_forbidden_check.lvs raises on Metal5 and Via4.
TOP_METAL = 4
...
CONFIG_MATRIX = [
    ...
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 4, "feed": "double"},
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 2, "feed": "double"},
    {"w": 5e-6, "l": 5e-6, "mmin": 3, "mmax": 4, "feed": "double"},
    # feed=same, including a band that does not start at Metal1
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 4, "feed": "same"},
    {"w": 5e-6, "l": 5e-6, "mmin": 2, "mmax": 4, "feed": "same"},
    # the PWell.block variant
    {"w": 5e-6, "l": 5e-6, "mmin": 1, "mmax": 4, "feed": "double", "subblock": 1},
    # non-square geometry, exercises the extractor's l->X / w->Y mapping
    {"w": 10e-6, "l": 20e-6, "mmin": 1, "mmax": 4, "feed": "double"},
]
...
def build_original():
    ...
    It used to be two static cells named cap_mom_lvs and cap_mom_same_lvs, one
    Recog.mom marker each. Their geometry really is the PCell's own output, but
    frozen: the cell names exist nowhere in this PDK or in ihp-sg13g2, no PCell
    emits them and no library holds them, so someone opening the first cap_cmomi
    testcase went looking for a cell that does not exist (issue #91), and nobody
    could regenerate it.

    It had already drifted. Compared cell by cell against the PCell at these
    parameters, the frozen copies match on every layer except NoMetFiller (160/0),
    which the current PCell emits and the snapshot does not have. Generating it
    here does not by itself prevent that: the layout is still a committed GDS and
    nothing re-runs this script in CI. What it does give is a recorded recipe, so
    ...
    double = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double")
    same = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="same")
...
def build_hier():
    ...
    pcell = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double")
...
def build_chain():
    ...
    cells = [_pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double") for _ in range(3)]
...
def build_short_same():
    ...
    pin to its own metal. Dropping a Via3 cut into the overlap shorts them. If
    ...
    broken = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="same")
    good = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double")
    trans = _row(layout, top, [broken, good])

    # The mmax and mmax-1 pins share one footprint; a Via3 in it merges them.
...
def build_all_dropped():
    ...
    cells = [_pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double") for _ in range(2)]
    _row(layout, top, cells)
    top.flatten(-1, True)
    _bridge_markers(layout, top)
...
def build_feed_none():
    ...
    A well formed cap sits next to it for the same reason as in the other
    malformed layouts. The case records that feed='none' IS extracted, so what it
    has to catch is the deck later refusing to extract it. On its own the cell
    would then hold no devices, drop out of the layout netlist, and go on
    reporting a match, which is the vacuity case 10 exists to close.
    """
    layout = _new_layout()
    top = layout.create_cell("cmomi_feed_none")

    none = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="none")
    good = _pcell(layout, w=5e-6, l=5e-6, mmin=1, mmax=4, feed="double")
    _row(layout, top, [none, good])
