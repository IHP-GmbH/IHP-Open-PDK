...
"""Run IHP 130nm CMOS Open Source PDK - SG13CMOS5L LVS cell regression.

SG13CMOS5L standard cells do not use _iso or _digisub variants
(nBuLay and DigiSub layers are forbidden in CMOS5L).
Only the base cell names are tested.
"""
...
def build_tests_dataframe(cells_dir, target_cell, cells):
    ...
    # CMOS5L: no _iso or _digisub variants (nBuLay/DigiSub are forbidden layers)
    return tc_df
...
def main(lvs_dir, cells_dir, output_path, target_cell, cells, workers_count):
    """
    Main function to run LVS regression for SG13CMOS5L std cells.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run IHP SG13CMOS5L LVS std-cell regression.",
        usage=USAGE,
    )
    parser.add_argument(
        "--cell",
    ...
    # Available cells - CMOS5L stdcell directory
    cells_dir = os.path.join(testing_dir, "testcases", "sg13cmos5l_cells")
