# LibreLane standard-cell-library configuration for the thick-oxide
# (3.3 V) cells inside the SG13CMOS5L PDK.
#
# The views themselves are symlinks into ihp-sg13g2 (see
# libs.ref/sg13cmos5l_stdcell_hv/): same GDS, same LEF, same liberty,
# same netlists as sg13g2_stdcell_hv. That is sound because CMOS5L
# symlinks the thick-oxide device models (cornerMOShv.lib,
# sg13g2_moshv_mod.lib, psp103.osdi ...) from the same G2 PDK, so the
# characterization the liberty carries is the characterization of this
# process module.
#
# Cell names therefore keep the sg13g2_hv_ prefix; unlike
# sg13cmos5l_stdcell they are not renamed, because renaming would fork
# every view from its single source of truth in ihp-sg13g2.
#
# Differences against libs.tech/librelane/sg13g2_stdcell_hv/config.tcl:
#   * the LIB dict, corner list and default corner live here -- this PDK
#     keeps them in the SCL config, not in the PDK-level config.tcl, so
#     no conditional block has to be patched into a shared file;
#   * VDD_PIN_VOLTAGE overrides the PDK-level 1.20 V;
#   * tracks.info next to this file is the CMOS5L routing grid
#     (M1-M4-TM1), not the G2 one.

set current_folder [file dirname [file normalize [info script]]]

# Technology lib -- the corners #1103 characterizes, each paired with the
# CMOS5L IO Liberty at the matching supply. Built the same way as the HV
# block in the G2 PDK config (a foreach with file-exists guards), so a
# corner that is not installed simply does not appear instead of making
# LibreLane fail on a missing path. No IO library is characterized against
# a 3.3 V core; these are the closest existing files, flagged for the
# maintainers exactly as in IHP-Open-PDK#1103.
set ::env(LIB) [dict create]
set _hv_corners [list]
foreach {_pat _hv _io} {
    "*_typ_3p30V_25C"   sg13cmos5l_stdcell_hv_typ_3p30V_25C   sg13cmos5l_io_typ_1p5V_3p3V_25C
    "*_fast_3p60V_m40C" sg13cmos5l_stdcell_hv_fast_3p60V_m40C sg13cmos5l_io_fast_1p65V_3p6V_m40C
    "*_slow_3p00V_125C" sg13cmos5l_stdcell_hv_slow_3p00V_125C sg13cmos5l_io_slow_1p35V_3p0V_125C
} {
    set _hvlib $::env(PDK_ROOT)/$::env(PDK)/libs.ref/$::env(STD_CELL_LIBRARY)/lib/$_hv.lib
    set _iolib $::env(PDK_ROOT)/$::env(PDK)/libs.ref/sg13cmos5l_io/lib/$_io.lib
    if { [file exists $_hvlib] } {
        if { [file exists $_iolib] } {
            dict set ::env(LIB) $_pat "$_hvlib $_iolib"
        } else {
            dict set ::env(LIB) $_pat "$_hvlib"
        }
        lappend _hv_corners "nom_[string trimleft $_pat {*_}]"
    }
}

# Corners
set ::env(STA_CORNERS) [join $_hv_corners " "]
set ::env(DEFAULT_CORNER) "nom_typ_3p30V_25C"
set ::env(TIMING_VIOLATION_CORNERS) "*"
unset _hv_corners _pat _hv _io _hvlib _iolib

# Thick-oxide core voltage (overrides the 1.20 V set at PDK level)
set ::env(VDD_PIN_VOLTAGE) "3.30"

# Synthesis mapping
 # Latch mapping
set ::env(SYNTH_LATCH_MAP) "$::env(PDK_ROOT)/$::env(PDK)/libs.tech/librelane/$::env(STD_CELL_LIBRARY)/latch_map.v"

 # MUX4 mapping
set ::env(SYNTH_MUX4_MAP) "$::env(PDK_ROOT)/$::env(PDK)/libs.tech/librelane/$::env(STD_CELL_LIBRARY)/mux4_map.v"

 # MUX2 mapping
set ::env(SYNTH_MUX_MAP) "$::env(PDK_ROOT)/$::env(PDK)/libs.tech/librelane/$::env(STD_CELL_LIBRARY)/mux2_map.v"

# Tri-state buffer mapping
set ::env(SYNTH_TRISTATE_MAP) "$::env(PDK_ROOT)/$::env(PDK)/libs.tech/librelane/$::env(STD_CELL_LIBRARY)/tribuff_map.v"

# Flip-flops map through dfflibmap like the thin-oxide library: all 8
# dfrbp/dfrbpq/sdfrbp/sdfrbpq flops and the 5 latches carry liberty AND
# layout views. sdfbbp_map.v remains available for designs that want
# every flop on the scan cell (see its header; it conflicts with the
# sdfbbp_1 entry in synth_exclude.cells, remove that line first).

# Placement site for core cells
# This can be found in the cell lef
set ::env(PLACE_SITE) "CoreSiteHV"
set ::env(PLACE_SITE_WIDTH) 0.48
set ::env(PLACE_SITE_HEIGHT) 7.14

# Welltap and endcap cells
# There are no endcap and welltie cells in ihp-sg13cmos5l
# thus set to undefined to skip insertion
#set ::env(WELLTAP_CELL) ""
#set ::env(ENDCAP_CELL) ""

# defaults (can be overridden by designs):
set ::env(SYNTH_DRIVING_CELL) "sg13g2_hv_buf_4"
set ::env(SYNTH_DRIVING_CELL_PIN) "X"
set ::env(OUTPUT_CAP_LOAD) "6.0"
set ::env(SYNTH_MIN_BUF_PORT) "sg13g2_hv_buf_1 A X"
set ::env(SYNTH_TIEHI_PORT) "sg13g2_hv_tiehi L_HI"
set ::env(SYNTH_TIELO_PORT) "sg13g2_hv_tielo L_LO"

# Fillcell insertion
set ::env(FILL_CELLS) "sg13g2_hv_fill_1 sg13g2_hv_fill_2 sg13g2_hv_fill_4 sg13g2_hv_fill_8"
set ::env(DECAP_CELLS) "sg13g2_hv_decap_*"

# Diode insertion
set ::env(DIODE_CELL) "sg13g2_hv_antennanp/A"

set ::env(GPL_CELL_PADDING) {0}
set ::env(DPL_CELL_PADDING) {0}

set ::env(CELL_PAD_EXCLUDE) "sg13g2_hv_fill_* sg13g2_hv_decap_*"

# PDN: the VSS rail is 0.44 um wide and symmetric about the row edge; the
# VDD pin shape is taller (0.535 um) and fully covers a 0.44 um strap.
set ::env(FP_PDN_RAIL_WIDTH) 0.44

# CTS
set ::env(CTS_ROOT_BUFFER) sg13g2_hv_buf_16
set ::env(CTS_CLK_BUFFERS) "sg13g2_hv_buf_8 sg13g2_hv_buf_4 sg13g2_hv_buf_2"

# Mirrored from sg13cmos5l_stdcell (themselves "a bit random ... from
# sky130"). The 3.3 V cells switch roughly 3x slower than the thin-oxide
# library; raise CLOCK_TRANSITION_CONSTRAINT at design level if CTS
# reports unattainable clock slew.
set ::env(MAX_FANOUT_CONSTRAINT) 10
set ::env(CLOCK_UNCERTAINTY_CONSTRAINT) 0.25
set ::env(CLOCK_TRANSITION_CONSTRAINT) 0.15
set ::env(TIME_DERATING_CONSTRAINT) 5
set ::env(IO_DELAY_CONSTRAINT) 20

# Tristate cells
set ::env(TRISTATE_CELLS) "sg13g2_hv_ebufn_* sg13g2_hv_einvn_*"

# TODO adjust threshold
set ::env(HEURISTIC_ANTENNA_THRESHOLD) 90
