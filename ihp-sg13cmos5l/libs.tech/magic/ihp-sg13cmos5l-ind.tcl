...
proc sg13cmos5l::inductor2_defaults {} {
...
proc sg13cmos5l::inductor3_defaults {} {
...
proc sg13cmos5l::ind_convert {parameters} {
...
proc sg13cmos5l::inductor2_convert {parameters} {
    return [ind_convert $parameters]
}

proc sg13cmos5l::inductor3_convert {parameters} {
    return [ind_convert $parameters]
}
...
proc sg13cmos5l::ind_dialog {device parameters} {
...
proc sg13cmos5l::inductor2_dialog {parameters} {
    sg13cmos5l::ind_dialog inductor2 $parameters
}

proc sg13cmos5l::inductor3_dialog {parameters} {
    sg13cmos5l::ind_dialog inductor3 $parameters
}
...
proc sg13cmos5l::ind_device {parameters} {
    set positions 2
    ...
    set keepout_area [sg13cmos5l::create_polygon 8 $kwidth $kwidth]
	...
	set wire_center [sg13cmos5l::create_wire $npoints $wdiam $wdiam]
	wire segment metal5 ${width}um {*}$wire_center
	...
	set d 2.0	;# minimum spacing of metal5
	...
	lappend xwire1 ${xpoint}um ${xpoint}um
	lappend xwire1 ${xend}um ${xpoint}um
	wire segment metal5 ${width}um {*}$xwire1 -noendcap
	...
	lappend xwire2 ${xpoint}um -${xpoint}um
	lappend xwire2 ${xend}um -${xpoint}um
	wire segment metal4 ${width}um {*}$xwire2 -noendcap
	...
	for {set i 1} {$i < $turns} {incr i} {
	    ...
	    box grow w ${width}um
	    box grow c 5
	    select area metal4,via4,metal5
	    copy n ${ypos}um
	...
	# plus metal4 spacing on either side
...
proc sg13cmos5l::inductor2_draw {parameters} {
    ...
    return [sg13cmos5l::ind_device $drawdict]
}
...
proc sg13cmos5l::inductor3_draw {parameters} {
    # Set a local variable for each rule in ruleset
    foreach key [dict keys $sg13cmos5l::ruleset] {
        set $key [dict get $sg13cmos5l::ruleset $key]
    }
...
proc sg13cmos5l::ind_check {device parameters} {
...
proc sg13cmos5l::inductor2_check {parameters} {
    return [sg13cmos5l::ind_check inductor2 $parameters]
}

proc sg13cmos5l::inductor3_check {parameters} {
    return [sg13cmos5l::ind_check inductor3 $parameters]
}
