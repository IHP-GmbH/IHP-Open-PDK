...
proc sg13cmos5l::bondpad_defaults {} {
    return {height 80 width 80 depth 5 class special shape octagon}
}

proc sg13cmos5l::bondpad_convert {parameters} {
...
proc sg13cmos5l::bondpad_dialog {parameters} {
...
proc sg13cmos5l::bondpad_draw {parameters} {
    ...
    set metal_edge [sg13cmos5l::create_polygon $npoints $width $height]
    set glass_edge [sg13cmos5l::create_polygon $npoints $pwidth $pheight]
    set metal_center [sg13cmos5l::create_wire $npoints $mwidth $mheight]
    ...
    # Create polygons
    polygon metal5 {*}$metal_edge
    polygon glass {*}$glass_edge

    # Create lower metals as wires 4um wide, matching the outer
    # edge of the pad metal.
    if {$depth > 1} {wire segment metal4 4um {*}$metal_center}
    if {$depth > 2} {wire segment metal3 4um {*}$metal_center}
    if {$depth > 3} {wire segment metal2 4um {*}$metal_center}
    if {$depth > 4} {wire segment metal1 4um {*}$metal_center}
	...
	set contact_points [sg13cmos5l::create_wire $npoints $mwidth $mheight]
	for {set i 1} {$i < $depth} {incr i} {
            set vn [- 5 $i]
...
proc sg13cmos5l::bondpad_check {parameters} {
    ...
    # Metal depth must be 1 to 5
    if {$depth < 1} {
	puts stderr "depth must be >= 1"
        dict set parameters depth 1
    }
    if {$depth > 5} {
	puts stderr "depth must be <= 5"
        dict set parameters depth 5
    }
