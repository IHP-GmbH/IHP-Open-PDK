...
proc sg13cmos5l::via_convert {parameters} {
    # Vias do not import from SPICE;  nothing to do.
}

proc sg13cmos5l::via_check {parameters} {
    ...
    set metallist {metal1 metal2 metal3 metal4 metal5}
...
proc sg13cmos5l::via_draw {parameters} {
    ...
    set metalwidthrule {0.16  0.20  0.20  0.20  1.64}
    set metalarearule  {0.09  0.144 0.144 0.144 0.0}
    set viawidthrule   {0.20  0.20  0.20  0.62}
    set spacingrule    {0.21  0.21  0.21  0.22}
    set b0overlaprule  {0.005 0.0   0.0   0.0}
    set b1overlaprule  {0.045 0.045 0.045 0.0}
    set t0overlaprule  {0.0   0.0   0.0   0.32}
    set t1overlaprule  {0.045 0.045 0.045 0.32}

    # These are IHP cut rules and are needed to determine what area
    # is required for a specific number of cuts.
    set cutsizerule    {0.19  0.19  0.19  0.42}
    set cutspacerule   {0.22  0.22  0.22  0.42}
    set cutspace2rule  {0.29  0.29  0.29  0.42}
    set cutborderrule  {0.005 0.005 0.005 0.10}
    ...
    if {[llength $orient] == 1} {
	if {$orient == "default"} {
	    set orient [lrepeat 2 horizontal vertical]
	} elseif {$orient == "inverse"} {
	    set orient [lrepeat 2 vertical horizontal]
	} else {
	    set orient [lrepeat 5 $orient]
	}
    ...
    set metalwidth [lrepeat 5 0]
    set metalheight [lrepeat 5 0]
    set viawidth [lrepeat 4 0]
    set viaheight [lrepeat 4 0]
...
proc sg13cmos5l::via1_draw {{dir default} {layers 2} {minarea 0}} {
   ...
   if {$dir == "vert"} {
      ...
      if {$layers > 1} {
         ...
         box grow n 0.045um
         box grow s 0.045um
	 sg13cmos5l::minareabox $minarea $dir
         paint m1
         popbox
      }
   } else {
