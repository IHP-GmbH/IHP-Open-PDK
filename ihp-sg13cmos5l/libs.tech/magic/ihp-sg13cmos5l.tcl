...
#-----------------------------------------------------
# Magic/TCL design kit for IHP ihp-sg13cmos5l
#-----------------------------------------------------
# Tim Edwards
# Revision 1.0	February 20, 2026
#-----------------------------------------------------
#
...
if [catch {set PDKPATH}] {set PDKPATH ${TECHPATH}/ihp-sg13cmos5l}
set PDKNAME ihp-sg13cmos5l
# "sg13cmos5l" is the namespace used for all devices
set PDKNAMESPACE sg13cmos5l
puts stdout "Loading ihp-sg13cmos5l Device Generator Menu ..."

# Initialize toolkit menus to the wrapper window

global Opts
namespace eval sg13cmos5l {}

# Set the window callback
if [catch {set Opts(callback)}] {set Opts(callback) ""}
set Opts(callback) [subst {sg13cmos5l::addtechmenu \$framename; $Opts(callback)}]
...
namespace eval sg13cmos5l {
    ...
    dict set ruleset mmetal_spacing   0.21      ;# Metal spacing rule (metal 2 to 4)
...
proc sg13cmos5l::addtechmenu {framename} {
   global Winopts Opts
   ...
   magic::add_toolkit_command $layoutframe "cap_cmomf" \
	    "magic::gencell sg13cmos5l::cap_cmomf" pdk1
   magic::add_toolkit_separator	$layoutframe pdk1
   ...
   ${layoutframe}.titlebar.mbuttons.file.toolmenu insert 4 command -label "Import SPICE" -command {sg13cmos5l::importspice}
...
source ${script_path}/ihp-sg13cmos5l-util.tcl	;# basic utility functions
source ${script_path}/ihp-sg13cmos5l-misc.tcl	;# tap rings
source ${script_path}/ihp-sg13cmos5l-via.tcl	;# via stacks
source ${script_path}/ihp-sg13cmos5l-fet.tcl	;# MOSFETs
...
source ${script_path}/ihp-sg13cmos5l-pad.tcl	;# Bond pad

# Make the script path available to procedures
set sg13cmos5l::script_path ${script_path}
