    ...
    def setupParams(self, params):
        ...
        if self.guardRingType != GuardRingType.NONE and \
           self.guardRingDistance < MIN_GUARD_RING_DISTANCE_UM:
            raise ValueError(
                f"guardRingDistance must be >= {MIN_GUARD_RING_DISTANCE_UM}um "
                f"(got {self.guardRingDistance}um)"
            )
        ...
        if self.guardRingType != GuardRingType.NONE:
            ...
            for s in self.getShapes():
                ...
                if isinstance(bbox, bool):
                    raise RuntimeError("Polygon shape bbox property returned a boolean. "
                                       "This should already have been fixed by https://github.com/IHP-GmbH/pycell4klayout-api/pull/3")
