...
|  | `lvsres` | `R1 N1 N2 lvsres layer=Metal1 l=1u w=5u R=20k`<br>`or`<br>`R1 N1 N2 20k $[lvsres] layer=Metal1 l=1u w=5u` | `layer`, `W`, `L` | [`R3`](#note-r3) | [CDL](./testcases/unit/res_devices/netlist/res_metal1.cdl) / [GDS](./testcases/unit/res_devices/layout/res_metal1.gds) |
...
- <a id="note-r3"></a>`R3`: Metal-resistor netlists accept either `w/l` or `width/length`. The mandatory `layer` parameter accepts `Metal1` through `Metal5`, `TopMetal1`, or `TopMetal2`. A bare `lvsres` is loaded as a deliberately unmatched device and produces a normal LVS violation. The resistance value is not used for LVS matching. Legacy `res_metal*` and `res_topmetal*` model names remain accepted on input, while extracted netlists always emit `lvsres layer=<metal>`.
