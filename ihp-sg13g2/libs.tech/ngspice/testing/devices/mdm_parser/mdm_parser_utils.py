...
import pandas as pd
from mdm_parser_const import DESIGN_PARAMETERS, UNIT_MULTIPLIERS, UNIT_REGEX
...
def extract_data_blocks(lines: List[str]) -> Generator[List[str], None, None]:
    ...
    for line in lines:
        ...
        if stripped == "BEGIN_DB":
            ...
            if current_block:  # Only yield non-empty blocks
                yield current_block
            in_block = False
            current_block = []
        elif in_block:
...
def parse_header_inputs(header_lines: List[str]) -> Set[str]:
    ...
    for line in input_lines:
        ...
        if stripped and not stripped.startswith("!"):
            # Take first word as variable name
...
def parse_design_parameters(header_lines: List[str]) -> Dict[str, float | str]:
    ...
    for line in header_lines:
        ...
        if in_values_section and stripped.startswith(("ICCAP_INPUTS", "ICCAP_OUTPUTS")): # something went wrong
        ...
        if in_values_section and stripped:
            ...
            if len(parts) == 2:
                key, value = parts
                # Handle both 'key' and 'key.subkey' formats
...
def parse_data_block(
    ...
    for line in block_lines:
        ...
        if not header_found:
            # Parse metadata before data header
...
def setup_global_logging():
    ...
    not exist, and initializes logging with a specific format and logging level.
