...
class MetricSpec:
    ...
    Optional gating/limit fields (absent -> no effect, current behavior preserved):
      min_limit / max_limit: fixed numeric bounds used when ss/ff columns are not
        available/usable (see RangeChecker._get_metric_bounds).
      valid_range: bias-window gating keyed by derived bias variable name, e.g.
        {"vbe": [0.7, 0.8], "vce": [0.5, 1.5]}. Points whose derived bias values fall
        outside the window are excluded from the pass/fail comparison (not counted).
        This is a flat, characteristic-agnostic window; see RangeChecker.mqa_ranges for
        the per-characteristic (keyed by input_data stem) equivalent used by HBT configs.
    """
